...
if (nrow(relevantResult) > 0) {
tableLegend <- c("GS" = "Geneset Collection : for more details see msigdb.org",
"FDR" = "False rate",
...
knitr::kable(data.frame("Name" = names(tableLegend), "Description" = tableLegend))
}
```
...
height.fig = max(2,2*nrow(mainPathways))

```


```{r plotGenesets, fig.width=20, fig.height=height.fig, fig.cap="Protein positions in ranked list for non-redundant Pathways"}
if (nrow(mainPathways) > 0) {
...
To generate the analysis the following R packages are used `r BiocStyle::Biocpkg("msigdbr")`, `r BiocStyle::Biocpkg("fgsea")` and `r BiocStyle::Githubpkg("protviz/prora", "prora")`
