...
```{r}
if (any(dim(res$ora) == 0)) {
  message("No significantly enriched pathways.")
} else {
  res$ora %>% 
...
```{r fig.cap="Heatmap of fold change patterns in significantly enriched pathways identified by sigORA.", fig.width=10, fig.height=20}
if (is_character(p1)) {
...
```{r}
colnames(res$data) <- c("Symbol", "Score")
res$data %>%
  mutate(`Score` = round(`Score`, 2)) %>% 
  DT::datatable(colnames = c("Symbol", "Score"), width = 500, style = "bootstrap")
```
