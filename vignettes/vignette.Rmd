...
vignette: >
  %\VignetteIndexEntry{fgczgseaora: Unifying methods on gene set enrichment}
...
data structures and other preconditions. `fgczgseaora` aims at providing a 
...
can be loaded using `library(fgczgseaora)`.
...
```{r getUniprot}
dd <- fgczgseaora::get_UniprotID_from_fasta_header(exampleContrastData)
#glimpse(dd)
```
...
to good scientific practice. In this spirit, `fgczgseaora` provides a reproducible, 
...
analysis and compiles `fgczgseaora's` in the same output directory.
...
The results are then used to generate the report provided by the `fgczgseaora` 
...
`system.file("run_scripts", package = "fgczgseaora")`]. 
